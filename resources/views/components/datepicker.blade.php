...
<section class="container mx-auto my-2">
        <div class="flex flex-wrap">
            <div class="w-full">
                <div class="">
                    <label for="date" class="block text-sm/6 font-medium text-gray-900 mb-2">
                        Choisissez une date :
                    </label>
                    ...
                    <div class="relative">
                        ...
                        <div id="datepicker-container" class="absolute mt-2 bg-white border border-stroke rounded-xl shadow-datepicker pt-5 hidden">
                            <div class="flex items-center justify-between px-5">
                                <button id="prevMonth" type="button" class="px-2 py-2 text-dark hover:bg-gray-2 rounded-md">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                <div id="currentMonth" class="text-lg font-medium text-dark-3 "></div>
                                <button id="nextMonth" type="button" class="px-2 py-2 text-dark  hover:bg-gray-2 rounded-md">
                                    <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
                            ...
                            <div class="grid grid-cols-7 gap-2 mt-6 mb-4 px-5">
                                <div class="text-center text-sm font-medium text-dark-3">Sun</div>
                                <div class="text-center text-sm font-medium text-dark-3">Mon</div>
                                <div class="text-center text-sm font-medium text-dark-3">Tue</div>
                                <div class="text-center text-sm font-medium text-dark-3">Wed</div>
                                <div class="text-center text-sm font-medium text-dark-3">Thu</div>
                                <div class="text-center text-sm font-medium text-dark-3">Fri</div>
                                <div class="text-center text-sm font-medium text-dark-3">Sat</div>
                            </div>
                            <div id="days-container" class="grid grid-cols-7 gap-2 mt-2 px-5"></div>
                            <!-- Buttons -->
                            <div class="flex justify-end mt-5 space-x-2.5 p-5 border-t border-stroke">
                                <button id="cancelButton" type="button" class="px-5 py-2.5 text-base font-medium text-primary rounded-lg border border-primary hover:bg-gray-100 transition">Cancel</button>
                                <button id="applyButton" type="button" class="px-5 py-2.5 text-base font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Apply</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            daysContainer.innerHTML += `<div class="flex items-center justify-center cursor-pointer w-[46px] h-[46px] text-dark-3 rounded-full hover:bg-blue-600 hover:text-white transition">${i}</div>`;
        }

        document.querySelectorAll('#days-container div').forEach(day => {
            day.addEventListener('click', function () {
                selectedDate = `${this.textContent}/${month + 1}/${year}`;
                dateValue = `${year}-${month + 1}-${this.textContent}`;
                document.querySelectorAll('#days-container div').forEach(d => d.classList.remove('bg-blue-600', 'text-white'));
    ...
    applyButton.addEventListener('click', function () {
        if (selectedDate && dateValue) {
            datepicker.innerHTML = selectedDate;
